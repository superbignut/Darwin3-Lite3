...
from opencv_zoo.models import MPPalmDet, MPHandPose
# used to estimate what the handpose is
class HandGesture:
    def __init__(self, detector:MPPalmDet, estimator:MPHandPose):
    ...
    def estimate(self, image, bbox=None, handedness_thr=0.6):
        ...
        for palm in palms:
            ...
            if handpose is not None:
                ...
                # filter left hands
                if handedness < handedness_thr: # 左右手阈值
                    continue

                # add bias
                bbox = handpose[0:4].reshape(2, 2) # 0:4 是手的框框
                ...
                gestures = np.append(gestures, self.__gesture_classification(landmarks)) # 手势 分类
                w, h = bbox[1] - bbox[0]
                area_list = np.append(area_list, w * h) # 保存的是手掌的面积，然后 找最大的用就行了
